	...
	public function to_json() {
		...
		if ( 'repeater' == $this->setting_type ) {
			$this->json['l10n']             = array(
				'untitled' => esc_html__( 'Untitled', 'masterclass' ),
			);
	...
	public function enqueue() {
		...
		if ( is_null( self::$_args_loaded ) ) {
			...
			$args = array(
				...
				'is_rtl'           => is_rtl(),
				'theme_default'    => esc_html__( 'Theme Default', 'masterclass' ),
				'reset'            => esc_html__( 'Reset this section settings', 'masterclass' ),
				'untitled'         => esc_html__( 'Untitled', 'masterclass' ),
				'confirm_reset'    => esc_html__( 'Do you want to reset this section settings?', 'masterclass' ),
				'list_font_weight' => array(
					''       => esc_html__( 'Default', 'masterclass' ),
					'normal' => _x( 'Normal', 'tophive-font-weight', 'masterclass' ),
					'bold'   => _x( 'Bold', 'tophive-font-weight', 'masterclass' ),
				),
				'typo_fields'      => top_hive()->customizer->get_typo_fields(),
				'styling_config'   => top_hive()->customizer->get_styling_config(),
				'devices'          => top_hive()->customizer->devices,
			...
			<?php if ( 'custom_html' != $this->setting_type ) { ?>
				...
				<?php if ( 'repeater' == $this->setting_type ) { ?>
					<div class="tophive--repeater-actions">
						<a href="#" class="tophive--repeater-reorder" data-text="<?php esc_attr_e( 'Reorder', 'masterclass' ); ?>" data-done="<?php _e( 'Done', 'masterclass' ); ?>"><?php _e( 'Reorder', 'masterclass' ); ?></a>
						<?php if ( false !== $this->addable ) { ?>
							<button type="button" class="button tophive--repeater-add-new"><?php _e( 'Add an item', 'masterclass' ); ?></button>
						<?php } ?>
					</div>
				<?php } ?>
			<?php } ?>
